...
root='./data/DT'
method='after'
model_name = 'deeplab_efficientnet-b3'#模型名称   
...
def train(test_fold): 
    ...
    #选择模型
    if model_name.lower() == 'deeplab_resnet18':
        model = smp.DeepLabV3Plus('resnet18', in_channels=1, classes=1, activation=None).to(device)
    if model_name.lower() == 'deeplab_resnet50':
        model = smp.DeepLabV3Plus('resnet50', in_channels=1, classes=1, activation=None).to(device)
    if model_name.lower() == 'deeplab_efficientnet-b2':
        model = smp.DeepLabV3Plus('efficientnet-b2', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'deeplab_efficientnet-b3':
        model = smp.DeepLabV3Plus('efficientnet-b3', in_channels=1, classes=1, activation=None,encoder_weights='imagenet').cuda()
    if model_name.lower() == 'deeplab_efficientnet-b6':
        model = smp.DeepLabV3Plus('efficientnet-b6', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'deeplab_resnet101':
        model = smp.DeepLabV3Plus('resnet101', in_channels=1, classes=1, activation=None).cuda()
    ...
    #             torch.save(model.state_dict(), join(save_path, str(epoch)+"_"+str(metric)+".pth"))
                ...
                if metric > best_metric:
                    best_metric = metric
                    best_metric_epoch = epoch + 1
                    torch.save(model.state_dict(), join(save_path, "best_model.pth"))
...
#             print(np.unique(y_true))
            ...
            FNSR+=fnsr
            HD+=hd
    print('Mean DSC:',100*DSC/n)
    print('Mean FPSR:',100*FPSR/n)
    print('Mean FNSR:',100*FNSR/n)
    print('Mean HD:',HD/n)
...
def test(test_fold):
    ...
    model_path = join(root,method, 'models',model_name,'fold'+str(test_fold),'best_model.pth')
    #选择模型
    if model_name.lower() == 'deeplab_resnet18':
        model = smp.DeepLabV3Plus('resnet18', in_channels=1, classes=1, activation=None).to(device)
    if model_name.lower() == 'deeplab_resnet50':
    ...
    if model_name.lower() == 'deeplab_efficientnet-b2':
        model = smp.DeepLabV3Plus('efficientnet-b2', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'deeplab_efficientnet-b3':
        model = smp.DeepLabV3Plus('efficientnet-b3', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'deeplab_efficientnet-b6':
        model = smp.DeepLabV3Plus('efficientnet-b6', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'deeplab_resnet101':
        model = smp.DeepLabV3Plus('resnet101', in_channels=1, classes=1, activation=None).cuda()
