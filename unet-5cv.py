...
root='./data/DT'
method='after'
model_name = 'unet_efficientnet-b3'#模型名称   
...
def train(test_fold): 
    ...
    #选择模型
    if model_name.lower() == 'unet_resnet18':
        model = smp.Unet('resnet18', in_channels=1, classes=1, activation=None).to(device)
    if model_name.lower() == 'unet_efficientnet-b2':
        model = smp.Unet('efficientnet-b2', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'unet_efficientnet-b3':
        model = smp.Unet('efficientnet-b3', in_channels=1, classes=1, activation=None,encoder_weights='imagenet').cuda()
    if model_name.lower() == 'unet_efficientnet-b6':
        model = smp.Unet('efficientnet-b6', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'unet_resnet101':
        model = smp.Unet('resnet101', in_channels=1, classes=1, activation=None).cuda()
    optimizer = torch.optim.Adam(model.parameters(), 1e-3)#优化器
    model_path = join(save_path, "best_ep250.pth")
    ...
    #             torch.save(model.state_dict(), join(save_path, str(epoch)+"_"+str(metric)+".pth"))
                ...
                if metric > best_metric:
                    best_metric = metric
                    best_metric_epoch = epoch + 1
                    torch.save(model.state_dict(), join(save_path, "best_ep300.pth"))
...
def test(test_fold):
    ...
    model_path = join( root,method,'models',model_name,'fold'+str(test_fold),'best_ep300.pth')
    #选择模型
    if model_name.lower() == 'unet_resnet18':
        model = smp.Unet('resnet18', in_channels=1, classes=1, activation=None).to(device)
    if model_name.lower() == 'unet_efficientnet-b2':
        model = smp.Unet('efficientnet-b2', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'unet_efficientnet-b3':
        model = smp.Unet('efficientnet-b3', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'unet_efficientnet-b6':
        model = smp.Unet('efficientnet-b6', in_channels=1, classes=1, activation=None).cuda()
    if model_name.lower() == 'unet_resnet101':
        model = smp.Unet('resnet101', in_channels=1, classes=1, activation=None).cuda()
