...
method='before'
suffix='.jpg'
model_name = 'upp_efficientnet-b3'#模型名称   
...
def train(test_fold): 
    ...
    #选择模型
    if model_name.lower() == 'upp_efficientnet-b3':
        model = smp.UnetPlusPlus('efficientnet-b3', in_channels=1, classes=1, activation=None,encoder_weights='imagenet').cuda()
    ...
    #             torch.save(model.state_dict(), join(save_path, str(epoch)+"_"+str(metric)+".pth"))
                ...
                if metric > best_metric:
                    best_metric = metric
                    best_metric_epoch = epoch + 1
                    torch.save(model.state_dict(), join(save_path, "best_ep250.pth"))
...
def predict_whole_img(test_img_dir,name,model,result_path):
...
def evaluation(infer_path):
    result_path=join(infer_path,model_name,method,'seg')
    gt_path=join(infer_path,'gt')
...
def infer_vis(infer_path):
    img_path=join(infer_path,'image')
    result_path=join(infer_path,model_name,method,'seg')
    gt_path=join(infer_path,'gt')
    vis_path=join(infer_path,model_name,method,'vis')
    if not os.path.exists(vis_path):
        os.makedirs(vis_path)
    for name in tqdm(ld(img_path)):
        if suffix in name:
            img = cv2.imread(join(img_path,name)) 
            pre = cv2.imread(join(result_path,name),0) 
            gt = cv2.imread(join(gt_path,name),0) 
...
def test(test_fold):
    ...
    model_path = join(root,method, 'models',model_name,'fold'+str(test_fold),'best.pth')
    #选择模型
    if model_name.lower() == 'upp_efficientnet-b3':
        model = smp.UnetPlusPlus('efficientnet-b3', in_channels=1, classes=1, activation=None).cuda()
    model.load_state_dict(torch.load(model_path))
    model.eval()
    with torch.no_grad():
...
def infer(infer_path,test_fold):
    print('start to predict...')
    model_path = join(root,method, 'models',model_name,'fold'+str(test_fold),'best.pth')
    #选择模型
    if model_name.lower() == 'upp_efficientnet-b3':
    ...
    model.eval()
    img_path=join(infer_path,'image')
    save_path=join(infer_path,model_name,method,'seg')
    for path in [img_path,save_path]:
        if not os.path.exists(path):
            os.makedirs(path)
    with torch.no_grad():
        for file in tqdm(ld(img_path)):
            if suffix in file:
                predict_whole_img(img_path,file,model,save_path)
    print('start to visualize...')
    infer_vis(infer_path)
    evaluation(infer_path)
    print('finished!')

def main():
    infer_path='./data/liver'
#     for i in range(5):
    infer(infer_path,3)
#         print(i)
#         print()
